...
/* -------------------------------------- 宏定义 -------------------------------------- */
#define CHIRP_LENGTH_BB_MAX_EXP	9 						 // 基带chirp点数相对于2的指数次幂
#define GUARD_LENGTH_BB_MAX	512              // 基带保护间隔
#define CHIRP_LENGTH_BB_MAX	512	             // 基带chirp
//循环前缀相关
...
static const float fir_corr_fall_chirp[CHIRP_LENGTH_BB_MAX * 2] = // 降chirp匹配滤波系数
        {
#include "mfsk_dem_para/fallbbchirp_512.par"
        };
static const float fir_corr_rise_chirp[CHIRP_LENGTH_BB_MAX * 2] = // 升chirp匹配滤波系数
        {
#include "mfsk_dem_para/risebbchirp_512.par"
        };
...
#pragma pack(pop)//恢复对齐状态
// 同步通道同步和解调相关变量
static MFSK_LONG_DEM_STATE sync_StateRecv = MFSK_LONG_FALLCHIRP_SYNC;    // 同步通道状态
...
}MPSK_LONG_DEM_STRUCT;
extern float mpsk_data2_buf[];
MPSK_LONG_DEM_STRUCT *mpsk_in_float = (MPSK_LONG_DEM_STRUCT *)mpsk_data2_buf;
/* ------------------------------------- 接口函数实现 ----------------------------------------- */
void MFSK_Long_DemInit(unsigned int nNum)
{
    memset(CorrPowSecond2, 0, sizeof(CorrPowSecond2));//4通道共用,相关需要前后各填CHIRP_LENGTH_BB_MAX-1个0
    MFSK_Long_DemSetSymbolNum(nNum);
    MFSK_Long_DemReset(1);
    //MPSK_DemInit();
}

void MFSK_Long_DemReset(unsigned char reset)//是否复位各通道接收状态
{
    // 清匹配滤波器状态
    ...
    if (reset == 1)
    {
        sync_StateRecv = MFSK_LONG_FALLCHIRP_SYNC;
    }
...
unsigned int MFSK_Long_DemGetSyncSampleNum(void)
{
    return mfsk_SyncSampleNum;
}

void MFSK_Long_DemResetSyncSampleNum(void)
{
    mfsk_SyncSampleNum = 0;
    sync_sample_num = 0;
}
//! 设置和获取MFSK基带符号长度
void MFSK_Long_SetMFSKSymbolBB(unsigned int length)
{
...
unsigned int MFSK_Long_DemSetFrameNum(unsigned int nNum)
{
...
unsigned int MFSK_Long_DemGetFrameNum(void)
{
    return mfsk_FrameNum;
}

void MFSK_Long_DemSetDecodeMode(unsigned char bIsRepeatCode)
{
    mfsk_bIsRepeatCode = bIsRepeatCode;
}

unsigned char MFSK_Long_DemGetDecodeMode(void)
{
    return mfsk_bIsRepeatCode;
}

MFSK_LONG_DEM_STATE MFSK_Long_DemSetRecvState(MFSK_LONG_DEM_STATE state)
{
    if (state == MFSK_LONG_RISECHIRP_SYNC)
    {
...
MFSK_LONG_DEM_STATE MFSK_Long_DemGetRecvState(void)
{
    return sync_StateRecv;
}

// 获取mfsk同步头相关系数
float MFSK_Long_DemGetSyncCorr(void)
{
    return sync_RiseChirpCorr;
}

unsigned int MFSK_Long_DemSetSymbolNum(unsigned int nNum)
{
    if (nNum == 0)
    {
...
unsigned int MFSK_Long_DemGetSymbolNum(void)
{
    return mfsk_SymbolNum;
}

// 设置循环前缀长度，单位毫秒
unsigned int MFSK_Long_DemSetCyclePrefixLen(unsigned int ms)
{
...
// 兼容MPSK同步
unsigned int MXSK_Long_DemSetDataSampleNum(unsigned int len)
{
...
MFSK_LONG_DEM_STATE MFSK_Long_DemRoll(unsigned char dem_mode, unsigned char is_DATA, char *pDecodedData, unsigned int *nOutLen, float *sync_bb_data)
{
    ...
    {
        case MFSK_LONG_FALLCHIRP_SYNC: // 搜索降chirp
            // 降chirp匹配滤波
            ...
            for (i = 0; i < CHIRP_LENGTH_BB_MAX; i++)
            {
                ...
                if ((sync_CorrPow[i] >= 64) && ((sync_CorrPow[i] * SYNC_GATE) > sync_SigPowAvg[i]))
                {
                    ...
                    sync_StateRecv = MFSK_LONG_RISECHIRP_SYNC;
                    wait_rise_chirp_num = 3;
                    break;
                }
            }
            break;
        case MFSK_LONG_RISECHIRP_SYNC: // 搜索升chirp
            wait_rise_chirp_num--;
            ...
            for (i = 0; i < CHIRP_LENGTH_BB_MAX; i++)
            {
                if ((sync_CorrPow[i + CHIRP_LENGTH_BB_MAX] >= 64) && (sync_CorrPow[i + CHIRP_LENGTH_BB_MAX] * SYNC_GATE) > sync_SigPowAvg[i]) // 过门限判决：y > x * 0.125
                {
                    ...
                    sync_StateRecv = MFSK_LONG_DEM_READY;
                    break;
                }
            }
            if (sync_StateRecv == MFSK_LONG_RISECHIRP_SYNC)
            {
        ...
        case MFSK_LONG_DEM_READY:
            ...
            sync_StateRecv = MFSK_LONG_DEM_FRAME;
            //LOG1("数据长度：%d", mxsk_demDataLen);
            break;
        case MFSK_LONG_DEM_FRAME:;
